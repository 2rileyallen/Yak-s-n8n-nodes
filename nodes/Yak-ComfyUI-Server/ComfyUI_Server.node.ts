...
export class ComfyUI_Server implements INodeType {
	...
	private static async waitForWebSocketResult(
		wsUrl: string,
		jobId: string,
		headers: IDataObject, // Pass headers to the function
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
			try {
				...
				// --- Step 1: Upload Files using Axios ---
				...
				const executePayload: IDataObject = {
					...
					// **NEW**: Add the output_node_id if it exists in the uiFile
				...
				const processResult = async (result: any) => {
					...
					if (outputAsFilePath) {
						...
						return { json: {}, binary: { [propertyName]: preparedData }, pairedItem: { item: itemIndex } };
					}
				};
